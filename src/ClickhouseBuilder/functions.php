...
if (!function_exists("raw")) {
    ...
    function raw(
        string $expr,
    ): \LaravelClickhouseEloquent\ClickhouseBuilder\Query\Expression {
        return new \LaravelClickhouseEloquent\ClickhouseBuilder\Query\Expression(
            $expr,
        );
    }
}
...
if (!function_exists("into_memory_table")) {
    ...
    function into_memory_table($builder, $structure = null): bool
    {
        ...
        if (
            is_null($tableName) &&
            $file instanceof \LaravelClickhouseEloquent\ClickhouseClient\Common\TempTable
        ) {
        ...
        if (is_null($structure)) {
            throw \LaravelClickhouseEloquent\ClickhouseBuilder\Exceptions\BuilderException::noTableStructureProvided();
        }
        ...
        $result = $builder
            ->newQuery()
            ->table($tableName)
            ->insertFile(array_keys($structure), $file, $format);
...
if (!function_exists("file_from")) {
    function file_from($file): \LaravelClickhouseEloquent\ClickhouseClient\Interfaces\FileInterface
    {
        if (is_string($file) && is_file($file)) {
            $file = new \LaravelClickhouseEloquent\ClickhouseClient\Common\File($file);
        } elseif (is_scalar($file)) {
            $file = new \LaravelClickhouseEloquent\ClickhouseClient\Common\FileFromString($file);
        }
